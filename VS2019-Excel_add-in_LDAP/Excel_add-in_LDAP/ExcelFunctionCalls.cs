...
public static class ExcelFunctionCalls
{
    ...
    [ExcelFunction(Description = "Location of user in a UDF Function")]
    public static string B2_LocationOf([ExcelArgument(Name = "UserName",
                                                      Description = "User as String to look up location")]
                                        string UserName)

    {

        var searcher = new DirectorySearcher("LDAP://" + UserName.Split('\\').First().ToLower())
        {

            Filter = "(&(objectCategory=User)(ObjectClass=person)(givenName:dn:=" + UserName.Split('\\').Last().ToLower() + "))"
        };
